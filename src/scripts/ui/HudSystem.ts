  ...
  updateHud(): void {
    for (const player of world.getAllPlayers()) {
      ...
      player.onScreenDisplay.setActionBar(
        `HP|${healthBar}|${formatPercent(currentHealth, maxHealth)} MP|${manaBar}|${formatPercent(stats.mana, stats.maxMana)} XP|${xpBar}|${formatPercent(stats.xp, xpNext)} LV${stats.level} AP${stats.attributePoints}`
      );
    }
  }
}
